    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (jwt != null && jwt.startsWith("Bearer")) {
            ...
            if (jwtUtils.isTokenValide(token)) {
                ...
                Claims donnees = jwtUtils.getData(token);

                // On recupere l'utilisateur dans la BDD en fonction de l'email du JWT
                UserDetails userDetails = userDetailsService.loadUserByUsername(donnees.getSubject());
